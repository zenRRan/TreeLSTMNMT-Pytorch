...
class DTTreeGRU(nn.Module):
    ...
    def forward(self, inputs, indexes, trees):
        ...
        for step in range(max_length):
            ...
            for b in range(batch_size):
                index = indexes[step, b]
                dt_state[b][index] = results[b]

        outputs, output_t = [], []
        for b in range(batch_size):
            output = [dt_state[b][idx] for idx in range(1,max_length)] + [dt_state[b][0]]
...
class TDTreeGRU(nn.Module):
    ...
    def forward(self, inputs, indexes, trees):
        ...
        for step in reversed(range(max_length)):
            ...
            for b in range(batch_size):
                index = indexes[step, b]
                td_state[b][index] = results[b]

        outputs, output_t = [], []
        for b in range(batch_size):
            output = [td_state[b][idx] for idx in range(1,max_length)] + [td_state[b][0]]
            outputs.append(torch.stack(output, 0))
